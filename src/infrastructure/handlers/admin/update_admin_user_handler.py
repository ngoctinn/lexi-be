...
def handler(event, context):
    ...
    try:
        ...
        logger.info("Update user handler invoked", extra={"context": {"user_id": user_id}})
    except KeyError:
    ...
    if not target_user_id:
        logger.warning("Missing user_id", extra={"context": {"user_id": user_id}})
        presenter = HttpPresenter()
        return presenter.present_bad_request("Missing user_id")
